...
# $parser->end_lines();
#
# Changelog: C.Guillon from 0.9.5:
# Fixed handling of lines begining with -/+.
# Terminate section on any unrocognized line
# Added support for === format (svn/ svk diff)
#
use strict;
...
sub next_line {
  ...
  # line parsing
  } elsif ($this->{IN_SECTION} && $line =~ /^ /) {
    push @{$this->{SECTION_STATE}{lines}}, $line;
  } elsif ($this->{IN_SECTION} && $line =~ /^-/) { # was: $line =~ /^-($|[^-])/ which is bogus
